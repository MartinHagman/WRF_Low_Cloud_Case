...
'''Detta skript plottar tvärsnitt samt tidsserier av variabler. Det fungerar för alla turbulensparametrar som har sina värden på de hela nivåerna.'''


TYPE_OF_RUN = 'REFERENCE'   #Kan vara 'REFERENCE', 'T_TO_Td', 'Wv_TO_Wvmax'
DATE = '18022600'
...
Filobjekt = N4.Dataset('/home/sm_marha/WRF_SCM/WRFV3.9.1.1_FM_FLX_KELLY_TEND_91_LEVELS/test/em_scm_xy/wrfout_d01_2018-02-27_00:00:00_T_LIQ_YSU_TOPD1_SOUNDING_SODANKYLA_QVAPOR_100')
#Filobjekt = N4.Dataset('/home/sm_marha/WRF_SCM/WRFV3.9.1.1_FM_FLX_KELLY_TEND/test/em_scm_xy/wrfout_d01_2018-02-18_00:00:00_GT1e-4_42h')
...
#fig1.savefig('/home/sm_marha/FIGURES/SINGLE_COLUMN/'+DATE +'Z/' + TYPE_OF_RUN + '/' + DATE +'z_TSKIN_Precip' + INTERPOLATION+ ' FEWER_LEVELS(1e-4)_42h')
#fig1.savefig('/home/sm_marha/FIGURES/SINGLE_COLUMN/'+DATE +'Z/91_LEVELS/' + TYPE_OF_RUN + '/' + DATE +'z_TSKIN_Precip' + INTERPOLATION)
...
if list(VARIABLE_DICT.keys())[0] == 'RH_2D':
    ...
    A = 1.1/44
    myplot = ax3.contourf(TIME_FULL, LEVELS_FULL, VARIABLE_DICT[list(VARIABLE_DICT.keys())[0]], N.arange(0, 1.1, A), norm = norm, extend = 'both', cmap ='Greens')
    contour_levels = N.arange(0, 1, 0.05)
    cp = plt.contour(TIME_FULL, LEVELS_FULL, VARIABLE_DICT[list(VARIABLE_DICT.keys())[0]], contour_levels, colors='black', linestyles='dotted', linewidth='0.2' )
    ...
    #A = 3e-1/20
    A = 70/20
    #myplot = ax3.contourf(TIME_FULL, LEVELS_FULL,VARIABLE_DICT[list(VARIABLE_DICT.keys())[0]], N.arange(0, 70, A), norm=norm, extend = 'both', cmap ='binary')
    

    '''Välj interpolering i fälten med contourf, eller plotta med scatter för att se exakt på vilka modellnivåer det finns moln'''
    #myplot = ax3.contourf(TIME_FULL, LEVELS_FULL, VARIABLE_DICT[list(VARIABLE_DICT.keys())[0]], [10, 20, 30, 40, 50, 100, 1000, 10000, 100000, 1000000], cmap ='binary', norm = LogNorm())
    myplot = ax3.contourf(TIME_FULL, LEVELS_FULL,VARIABLE_DICT[list(VARIABLE_DICT.keys())[0]],[0.01, 0.1, 1, 10, 20, 30, 40, 50, 60, 70], cmap ='binary', norm = LogNorm())
    ...
    ax3.scatter(X, MASSLEVELS[0, :], marker= '>', color='black', s=20)
    plt.xlim(0, N.max(TIME))
    plt.ylim(1000,1500)     #Finns ylim längre ner i skriptet också
...
VARIABLE = 'EXCH_H' #omdefinierar VARIABLE för titeln
...
if VARIABLE != 'CLDFRA' and VARIABLE != 'RH':
    ...
    cbar_ax = fig2.add_axes([0.92, 0.30, 0.01, 0.58])
    #cbar = fig2.colorbar(myplot, ticks=, cax=cbar_ax)#, format = "%8.1e")
    cbar = fig2.colorbar(myplot, cax=cbar_ax, ticks =[0.01, 0.1, 1, 10, 100, 100])#, format = "%8.1e")
...
if VARIABLE == 'RH':
     ...
     ax3.set_title(VARIABLE + ' ' + TYPE_OF_RUN +' '+ DATE)

elif VARIABLE == 'CLDFRA':

    ax3.set_title(Filobjekt.variables[VARIABLE].description + ' ' + TYPE_OF_RUN + ' ' + DATE, fontsize=16)
    

elif VARIABLE == 'RTHRATEN' or VARIABLE == 'RTHBLTEN':
    
    ax3.set_title(Filobjekt.variables[VARIABLE].description + ' ' + TYPE_OF_RUN + ' ' + DATE + ' (K/s)', fontsize=16)
    

elif VARIABLE == 'EXCH_H' or VARIABLE == 'EXCH_M':
    
    #ax3.set_title(Filobjekt.variables[VARIABLE].description + ' ' + TYPE_OF_RUN + ' ' + DATE + ' (m2/s)', fontsize=16)
    ax3.set_title('LIQW 100% YSU TOPD=1 SOUNDING FROM SODANKYLÄ K')
    
    
else:

    plt.title(Filobjekt.variables[VARIABLE].description + ' ' + TYPE_OF_RUN + ' ' + DATE + ' (g/kg)', fontsize=16)
...
ax3.set_xticklabels(ALLA_DATUM_3h, rotation=45, ha='right', fontsize=15)
...
#fig2.savefig('/home/sm_marha/FIGURES/SINGLE_COLUMN/'+DATE +'Z/91_LEVELS/' + TYPE_OF_RUN + '/' + DATE +'z_CROSSSECTION' + VARIABLE + '_THETA'  + INTERPOLATION + ' FEWER_LEVELS(1e-4)_42h')
...
plt.title(TYPE_OF_RUN +' '+ DATE)
#plt.title('LIQW 100% MYNN SOUNDING FROM SODANKYLÄ')
...
#fig3.savefig('/home/sm_marha/FIGURES/SINGLE_COLUMN/' + DATE +'Z/91_LEVELS/' + TYPE_OF_RUN + '/' + DATE +'z_FLUXES' + INTERPOLATION+ ' FEWER_LEVELS(1e-4)_42h')
