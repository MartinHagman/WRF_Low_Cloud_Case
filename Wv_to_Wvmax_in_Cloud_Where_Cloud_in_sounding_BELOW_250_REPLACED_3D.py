...
LANDMASK = Filobjekt.variables['LANDMASK'][0, :, :]
...
'''
#Ersätter Theta med theta från sonderingen i de lägsta 350 metrarna. QCLOUD skulle kunna vara något annat, bara området vi loopar över.
    ...
    for i in range(250, QCLOUD.shape[2]):
        if LANDMASK[j, i] == 1:
            for k, value in enumerate(MASSLEVELS[:, j, i]):
                if value < 350:                 
                    #print(value, '   ', 'Ruppe')
                    for place, height in enumerate(HEIGHT_TER_SONDERING):
                        if height >= value:
                            #print(THETA_SONDERING[place], '   ', 'Ett')
                            THETA[k, j, i] = THETA_SONDERING[place]
                            break
                        elif height >= value-2:
                            #print(THETA_SONDERING[place], '   ', 'Två')
                            THETA[k, j, i] = THETA_SONDERING[place]                        
                            break'''
...
for j in range(500, QCLOUD.shape[1]):
    ...
    for i in range(250, QCLOUD.shape[2]):
        for k, value in enumerate(MASSLEVELS[:, j, i]):
            if value >1000 and value <1320:
                THETA_BELOW_CLOUD[k, j, i] = THETA[k-1, j, i]
...
for j in range(500, QCLOUD.shape[1]):
    ...
    for i in range(250, QCLOUD.shape[2]):
        ...
        for k, value in enumerate(MASSLEVELS[:, j, i]):           
            ...
            if value >1000 and value <1320 and n==0:
                ...
                n+=1
            elif value >1000 and value <1320:
                LIQ_WATER[k, j, i] = (THETA[k, j, i] - THETA_BELOW_CLOUD)/((2.260e6*THETA[k, j, i])/(1004*T_K[k, j, i]))
                THETA_LIQ[k, j, i] = THETA_BELOW_CLOUD
                
                #THETA[k, j, i] = THETA_LIQ[k, j, i] + (2.260e6/1004)* LIQ_WATER[k, j, i]     #Denna rad är till för att ta bort stabiliteten där molnet har initialiserats, dvs sätta Theta konstant med höjden.
                #print(THETA_BELOW_CLOUD)
                #print(LIQ_WATER[k, j, i])
                #print(THETA_LIQ[k, j, i])
                #print(THETA[k, j, i])
...
Filobjekt = N4.Dataset('/nobackup/smhid12/sm_marha/2018-02-27_00z_91_LEVELS_Clouds_Where_Clouds_In_Sounding_YSU_TOPD1_100_Reduced_Stability_in_clouds_and_dryer_below250/run/wrfinput_d01', mode='r+')
...
plt.plot(T_C[0:30, j, i], MASSLEVELS[0:30, j, i])
plt.plot(T_DAGGPUNKT[0:30, j, i], MASSLEVELS[0:30, j, i])

plt.show()

#for j in range(563, 564):
for j in range(500, QCLOUD.shape[1]):
    #for i in range(352, 353):
    for i in range(250, QCLOUD.shape[2]):
        for k, value in enumerate(MASSLEVELS[:, j, i]):
            #if value >= 1e-5:
